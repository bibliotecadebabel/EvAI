...
class CommandExperimentCifar_Restarts():
    ...
    def __generateNetworks(self):
        ...
        print("total network kids: ", (len(self.__networks) - 1))
    ...
    def __getNodeByNetwork(self, network):
    ...
    def __getDirectionsNode(self, network):

        directions = None
        node = self.__getNodeByNetwork(network)

        if node is not None:
            directions = node.objects[0].objects[0].direction
        ...
        if self.__settings.disable_mutation == False:
            ...
            for j in range(1, self.__settings.epochs+1):
                ...
                for i  in range(1, len(self.__networks)):
                    print("Training net #", i, " - directions: ", self.__getDirectionsNode(self.__networks[i]))
    ...
    def __generateNewSpace(self):
        ...
        while stop == False:
            ...
            newSpace = DNA_Graph(center=newCenter, size=oldSpace.size, dim=(oldSpace.x_dim, oldSpace.y_dim),
                                    ...
                                    type_add_layer=oldSpace.version, creator=Creator_nm, 
                                    num_morphisms=oldSpace.creator.num_morphisms, selector=oldSpace.creator.Selector)
