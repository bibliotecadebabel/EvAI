...
class AddExitFilterMutation(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        if cuda == True:
            resized = torch.zeros(1, shape[1], shape[2], shape[3], shape[4]).cuda()
        else:
            resized = torch.zeros(1, shape[1], shape[2], shape[3], shape[4])
...
class AddEntryFilterMutation(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        if cuda == True:
            resized = torch.zeros(shape[0], 1, shape[2], shape[3], shape[4]).cuda()
        else:
            resized = torch.zeros(shape[0], 1, shape[2], shape[3], shape[4])
...
class RemoveExitFilterMutation(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        oldFilter.resize_(shape[0]-1, shape[1], shape[2], shape[3], shape[4])
        oldBias.resize_(shape[0]-1)
...
class RemoveEntryFilterMutation(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        if cuda == True:
            resized = torch.zeros(shape[0], shape[1]-1, shape[2], shape[3], shape[4]).cuda()
        else:
            resized = torch.zeros(shape[0], shape[1]-1, shape[2], shape[3], shape[4])
...
class AddDimensionKernel(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        if cuda == True:
            resized_1 = torch.zeros(shape[0], shape[1], shape[2], shape[3], 1).cuda()
            resized_2 = torch.zeros(shape[0], shape[1], shape[2], 1, shape[4]+1).cuda()
        else:
            resized_1 = torch.zeros(shape[0], shape[1], shape[2], shape[3], 1)
            resized_2 = torch.zeros(shape[0], shape[1], shape[2], 1, shape[4]+1)

        oldFilter = torch.cat((oldFilter, resized_1), dim=4)
        oldFilter = torch.cat((oldFilter, resized_2), dim=3)
        ...
        for out_channel in range(shape[0]):
            for in_channel in range(shape[1]):
                for kernel_depth in range(shape[2]):
                    oldFilter[out_channel][in_channel][kernel_depth][shape[3]] = oldFilter[out_channel][in_channel][kernel_depth][shape[3]-1].clone() 
                    for x in range(shape[3]+1):
                        oldFilter[out_channel][in_channel][kernel_depth][x][shape[4]] = oldFilter[out_channel][in_channel][kernel_depth][x][shape[4]-1].clone()
...
class RemoveDimensionKernel(Mutation):
    ...
    def doMutate(self, oldFilter, oldBias, newNode, cuda):
        ...
        if cuda == True:
            resized = torch.zeros(shape[0], shape[1], shape[2], shape[3]-1, shape[4]-1).cuda()
        else:
            resized = torch.zeros(shape[0], shape[1], shape[2], shape[3]-1, shape[4]-1)
        ...
        for out_channel in range(shape[0]):
            for in_channel in range(shape[1]):
                for kernel_depth in range(shape[2]):
                    for kernel_x in range(shape[3]-1):
                        for kernel_y in range(shape[4]-1):
                            resized[out_channel][in_channel][kernel_depth][kernel_x][kernel_y] = oldFilter[out_channel][in_channel][kernel_depth][kernel_x][kernel_y].clone()
