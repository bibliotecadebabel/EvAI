...
import Geometric.Directions.DNA_directions_pool_duplicate as direction_dna
import torch
...
def execute_mutation(old_network, new_adn):
    ...
    if length_new_adn == length_old_adn:
        ...
        index_layer = __getTargetIndex(old_adn=old_network.adn, new_adn=new_adn, direction_function=direction_dna.add_layer)

        if index_layer == None:
            index_layer = __getTargetIndex(old_adn=old_network.adn, new_adn=new_adn, direction_function=direction_dna.add_pool_layer)

        __init_add_layer_mutation(old_network=old_network, network=network, lenghtold_adn=length_old_adn, added_layer_index=index_layer)

    elif length_old_adn > length_new_adn: # remove layer
        #print("remove layer mutation")
        index_layer = __getTargetIndex(old_adn=old_network.adn, new_adn=new_adn, direction_function=direction_dna.remove_layer)
...
def __init_add_layer_mutation(old_network, network, lenghtold_adn, added_layer_index):
...
def __getTargetIndex(old_adn, new_adn, direction_function):
    ...
    for i in range(len(old_adn)):
        ...
        if old_adn[i][0] == 0: #check if is conv2d
            generated_dna = direction_function(indexConv2d, old_adn)
